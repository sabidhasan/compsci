...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        distances = [distance(restaurant_location(restaurant), c) for c in centroids]
        min_dist_key = distances.index(min(distances))
        if not min_dist_key in ret:
            ret[min_dist_key] = []
        ret[min_dist_key].append(restaurant)
...
def find_predictor(user, restaurants, feature_fn):
    ...
    def predictor(restaurant):
        b = (S_xy / S_xx)
        return (b * feature_fn(restaurant)) + (mean(ys) - b * mean(xs))
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max((find_predictor(user, reviewed, ff) for ff in feature_fns), key=lambda x: x[1])[0]
    # END Question 8
