...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        curr_tile = self.place
        while curr_tile is not hive:
            if curr_tile.bees:
                return random_or_none(curr_tile.bees)
            curr_tile = curr_tile.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        curr_tile, tile_dist = self.place, 0
        while curr_tile is not hive and tile_dist <= self.max_range:
            if curr_tile.bees:
                return random_or_none(curr_tile.bees)
            curr_tile, tile_dist = curr_tile.entrance, tile_dist + 1
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        # Define places
        furthest_bee, curr_tile, tile_dist = None, self.place, 0

        while curr_tile is not hive:
            if curr_tile.bees and tile_dist >= self.min_range and not furthest_bee:
                furthest_bee = random_or_none(curr_tile.bees)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.true_queen_instantiated:
            QueenAnt.true_queen_instantiated = True
    ...
    def action(self, colony):
        ...
        curr_tile = self.place.exit
        while curr_tile:
            # Update all ants damage
            if not curr_tile.ant:
                continue
            
            curr_tile.ant.damage *= 2
            self.bonuses.append(curr_tile.ant)

            if curr_tile.ant.contained_ant:
                # To handle the Tank ants
                curr_tile.ant.contained_ant.damage *= 2
                self.bonuses.append(curr_tile.ant.contained_ant)

            curr_tile = curr_tile.exit

        curr_tile = self.place.entrance
        while curr_tile:
            # Update all ants damage
            if not curr_tile.ant:
                continue
    ...
    def reduce_armor(self, amount):
        ...
        """
        self.armor -= amount
        if amount <= 0 and self.is_true_queen:
            bees_win()
