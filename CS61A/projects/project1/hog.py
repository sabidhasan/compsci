...
def free_bacon(score):
    ...
    return max((2 * (score // 10)) - (score % 10), 1)
...
def is_swap(player_score, opponent_score):
    ...
    p_tens = (player_score % 100) // 10

    o_ones = opponent_score % 10
    o_tens = (opponent_score % 100) // 10

    return abs(p_ones - p_tens) == abs(o_ones - o_tens)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if player == 0:
            ...
            num_dice_to_roll = strategy(score0, score1)
            
            # Roll the dice
            score0 += take_turn(num_dice_to_roll, score1, dice)
            
            # Swap if needed
            if is_swap(score0, score1):
                score0, score1 = score1, score0
        else:
            num_dice_to_roll = strategy(score1, score0)

            score1 += take_turn(num_dice_to_roll, score0, dice)

            if is_swap(score1, score0):
                score1, score0 = score0, score1

        # Switch players, and say        
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def a(score0, score1):
        ...
        if score - previous_score > previous_high:
            ...
            print("%i point(s)! That's the biggest gain yet for Player %i" % (new_high, who))
        else:
            new_high = previous_high

        return announce_highest(who, previous_high=new_high, previous_score=score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # averaged_func is function that averages roll_dice being called num_sample times 
    averaged_func = make_averaged(roll_dice, num_samples)

    max_score = 0
    max_num_dice = 0

    for num_dice in range(10):
        # Get the score from current number of dice (+1 for 0 indexing)
        score = averaged_func(num_dice + 1, dice)

        if score > max_score:
            max_score = score
            max_num_dice = num_dice + 1
