...
def store_digits(n):
    ...
    if n < 10:
        return Link(n)
    return Link(int(str_n[0]), store_digits(int(str_n[1:])))
...
def is_bst(t):
    ...
    if len(t.branches) == 2:
        ...
        return bst_max(left_branch) <= t.label and bst_min(right_branch) >= t.label and all(is_bst(b) for b in t.branches)
