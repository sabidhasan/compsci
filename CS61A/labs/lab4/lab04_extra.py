...
def flatten(lst):
    ...
    ret = []
    for i in lst:
        ret += flatten(i) if type(i) == list else [i]
    return ret
...
def put_piece(board, max_rows, column, player):
    ...
    try:
        col = [get_piece(board, i, column) for i in range(max_rows)]
        first_empty_spot = max_rows - col[::-1].index('-') - 1
    except ValueError:
        return -1, board
    
    # Now create a new board
    board[first_empty_spot] = replace_elem(board[first_empty_spot], column, player)
    return first_empty_spot, board
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    False
    """
    row_pieces = ''.join(get_piece(board, row, i) for i in range(max_cols))
    return player * num_connect in row_pieces
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    False
    """
    col_pieces = ''.join(get_piece(board, i, col) for i in range(max_rows))
    return player * num_connect in col_pieces
