...
def replace_leaf(t, old, new):
    ...
    new_label = label(t).replace(old, new) if is_leaf(t) else label(t)
...
def balanced(m):
    ...
    False
    """
    # If the mobile is a weight, we return its size
    if is_weight(m):
        return size(m)

    # Get total weight for left and right
    left, right = [total_weight(end(x)) * length(x) for x in m[1:]]
    
    # If not equal, they are not balanced
    if left != right:
        return False
...
def totals_tree(m):
    ...
    if is_side(m):
        return totals_tree(m[2])

    left_branch = totals_tree(m[1])
    right_branch = totals_tree(m[2])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw):
        nonlocal balance, pwatts
        if len(pwatts) == 3:
            return f"Your account is locked. Attempts: " + str(pwatts)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint_wd(amount, pw):
        if pw != new_password and pw != old_password:
            # Pass it on if it doesn't match to the original withdraw to deal with
            return withdraw(amount, pw)
        else:
            # Perform a withdraw as PW is correct
...
def generate_paths(t, x):
    ...
    if label(t) == x:
        # This path led to what we need, so yield it
        yield paths
    
    # Look thru child branches
    for branch in branches(t):
        for path in generate_paths(branch, x):
            # Concatenate to existing path
            yield paths + path
...
def mul_interval(x, y):
    ...
    p2 = lower_bound(x) * lower_bound(y)
